  ...
  function fetch() {
    ...
    function next(i) {
      ...
      GM_xmlhttpRequest({
          ...
          try {
            ...
            if (f.t === "json") {
              ...
              (obj.result?.data || obj.items || []).forEach(it => {
                ...
                if (t) results.push({ t, l: it.url || it.link || "", d: fmt(it.ctime ? (it.ctime > 1e12 ? it.ctime : it.ctime * 1000) : it.pubDate), s: f.name });
              });
            } else {
              ...
              if (!p.querySelector("parsererror")) {
                p.querySelectorAll("item, entry").forEach(it => {
                  ...
                  const dd = it.querySelector("pubDate, published, updated")?.textContent?.trim() || "";
        ...
        onerror: () => next(i + 1), ontimeout: () => next(i + 1)
      });
    }
    next(0);
  }
